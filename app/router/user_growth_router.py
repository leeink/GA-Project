...
async def get_dashboard_page(request: Request, year: int):
...
async def get_yearly_api(year: int, db: AsyncSession = Depends(get_db)):
    return {"data": await find_yearly_user_growth(db, year)}

@router.get("/api/quarterly/{year}")
async def get_quarterly_api(year: int, db: AsyncSession = Depends(get_db)):
    return {"data": await find_quarterly_compare_data(db, year)}

@router.get("/api/monthly/{year}")
async def get_monthly_api(year: int, db: AsyncSession = Depends(get_db)):
